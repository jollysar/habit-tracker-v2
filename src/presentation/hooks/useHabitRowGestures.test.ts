...
describe("habitRowSwipeAction", () => {
  it("reveals delete after a deliberate right swipe", () => {
    expect(habitRowSwipeAction({ x: 20, y: 30 }, { x: 90, y: 34 })).toBe("reveal-delete");
  });

  it("closes delete after a deliberate left swipe", () => {
    expect(habitRowSwipeAction({ x: 100, y: 30 }, { x: 30, y: 34 })).toBe("close-delete");
  });
